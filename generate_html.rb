...
calling_cards.each do |calling_card|
  ...
  index_html << "<div class='col-sm-4' style='padding-left: 25px; padding-right: 25px; margin-bottom: 50px; text-align: center;'><h4><a href='#{calling_card['directory']}.html'>#{calling_card['title']}</a></h4>"
  # Gif Exists
  if File.exists?("#{dir}/#{calling_card['directory'].downcase.gsub('_', '-')}.gif")
    index_html << "<img style='width: 400px' src=\"../#{dir}/#{calling_card['directory'].downcase.gsub('_', '-')}.gif\"></img>"
  elsif File.exists?("#{dir}/combined_images/final_001.png") # Combined Image Exists
...
File.open('html/index.html', 'w') { |f| f.write(index_html) }
...
calling_cards.each do |calling_card|
  ...
  if calling_card['type'] == 'webm'
    num_images = Dir[File.join("calling_cards/#{calling_card['directory']}/combined_images", '**', '*')].count { |file| File.file?(file) }
    ...
    calling_card_html << "<div class='row' style='text-align: center;'><div class='col-sm-4' style='padding-left: 25px; padding-right: 25px; margin-bottom: 50px; text-align: center;'><img src=\"../calling_cards/#{calling_card['directory']}/#{calling_card['title'].downcase.gsub(' ', '-')}.png\" /></div></div>"
  end

  File.open("html/#{calling_card['directory']}.html", 'w') { |f| f.write(calling_card_html) }
end
